...
import web.automation.core.exception.image.NoImageForModificationException;
...
public class WebBufferedImage {
	...
	public WebBufferedImage addTitleTooltip (By... locators){
		BrowserElement AutoElement;
		for (int i=0; i<locators.length; i++){
			AutoElement = browser.find().element(locators[i]);
			//currentImage = WebImage.addTitleTooltip2Btm (currentImage,SCREENTEXTSIZE,AutoElement.getWebElement());
			addImage(WebImage.createTitleTooltip(AutoElement.getText()));
		}
		return this;
	}
	...
	public WebBufferedImage addElementText (By... locators){
		...
		for (int i=0; i<locators.length; i++){
			AutoElement = browser.find().element(locators[i]);
			addImage(WebImage.createElementText(AutoElement.getWebElement()));
			//currentImage = WebImage.addElementText2Btm (currentImage,AutoElement.getWebElement());
		}
		return this;
	}
