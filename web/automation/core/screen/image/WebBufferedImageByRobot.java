...
import web.automation.core.browser.handle.WindowMode;
...
public class WebBufferedImageByRobot extends WebBufferedImage{
	...
	public WebBufferedImageByRobot takeScreen(By crop){
		BrowserElement AutoElement = browser.waitFor().found(crop);
		currentImage = browser.robotSetup().robot().createScreenCapture(getElementRec(AutoElement));
		locateElement=false;
		return this;
	}
	...
	public WebBufferedImageByRobot takeTooltip(By tooltip,int heightMultipier){
		BrowserElement AutoElement = browser.waitFor().found(tooltip);
		int y = AutoElement.getLocation().getY()+browser.robotSetup().robotOffset().getY();
	...
	public WebBufferedImageByRobot subImage(By area){
		BrowserElement AutoElement = browser.waitFor().found(area);
		subImage(getElementRec(AutoElement));
		return this;
	}
	**/
	...
	public WebBufferedImageByRobot subImageKeepWidth(By area){
		...
		BrowserElement AutoElement = browser.waitFor().found(area);
		Rectangle rec = getElementRec(AutoElement);
	...
	**/
	...
	private Rectangle getElementRec(BrowserElement AutoElement){
		...
		int x = AutoElement.getLocation().getX()+offset.getX();
		int y = AutoElement.getLocation().getY()+offset.getY();
